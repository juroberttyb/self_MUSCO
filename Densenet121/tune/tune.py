...
if __name__ == '__main__':
    net = arch.densenet121(pretrained = True)
    info(net)

    net = fa.densenet121(net) # .cuda()
    state_dict = torch.load('71.412r0.543.pth', map_location = 'cpu') # , map_location = 'cuda:0')
    net.load_state_dict(state_dict)
    # info(net)

    net = rf.densenet121(net).cuda()
    info(net)
    ...
    for epoch in range(5000):
        ...
        if prec1 > best_prec1:  
            best_prec1 = prec1
            torch.save(net.state_dict(), './model2/epoch: ' + str(epoch) + ', prec1: ' + str(prec1) + '.pth')
